...
nnoremap <silent> <plug>(SubversiveSubstituteMotion) :<c-u>call subversive#onPreSubstitute(v:register, 0)<cr>:set opfunc=subversive#substituteMotion<cr>g@

" Here we trigger the substitute motion and explicitly use the visual mode marks, but then inside subversive#substituteMotion we again use the visual mode marks
" You might wonder why do we need to use < and > inside subversive#substituteMotion if we are already passing them here?  Won't ] and [ be set to > and < anyway?
...
xnoremap <silent> <plug>(SubversiveSubstituteMotion) <esc>`<:<c-u>call subversive#onPreSubstitute(v:register, 1)<cr>:set opfunc=subversive#substituteMotion<cr>g@`>
